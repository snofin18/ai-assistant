...
- **依赖**：016　**预估**：M　**难度**：M
- **write scope**：`crates/platform/windows/src/uia/**`、`.../src/window/**`
- **关联**：`plans/stage-1-pilots.md` 批次表 A2（1a）、`docs/wbs-overview.md` §6（DoD）

**目标**

`platform/windows`：UIA provider（树快照、selector 链解析、read_text、set_value、edit_text、invoke_action、bounds、fingerprint、window 枚举与状态）。

**write scope**（本卡独有部分，完整列表见 plan 批次表）

`crates/platform/windows/src/uia/**`、`.../src/window/**`
