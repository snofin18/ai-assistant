...
> **本卡记录的形态（2026-09-24 治理回填）**：TASK-011 先后经历多轮（`TASK-103` 重做 → 修订硬化 `335e1ad`
> → 修复轮 → 收尾轮 `5257dc6` → `gate-selftest` follow-up），每轮以 `### UPDATE <日期>` 块**追加**
> （块内自编 `§1`~`§11`），**没有**逐轮回填到下面这 9 节骨架里。
> 骨架 9 节标题齐全（`card-check` 判据 ③ 要求）；各节内容的**落点**见每节下方的一行指引 ——
> 只写「去哪看」而**不复制内容**，避免同一事实出现两个落点（ADR-0030 的整条动机）。

### 1. 约束回执（**动手前**填，`AGENTS.md` §3 的固定格式）

> 落点：`UPDATE 2026-09-24` §1（收尾轮回执）+ `UPDATE 2026-09-24b` §1（follow-up 回执）。
...
> 落点：`UPDATE 2026-09-23b` §2 / `UPDATE 2026-09-23c` §2 / `UPDATE 2026-09-24` §2 / `UPDATE 2026-09-24b` §2。

### 3. 验收输出摘要（命令 → 结果，全绿 / 失败项）

> 落点：`UPDATE 2026-09-23c` §3 / `UPDATE 2026-09-24` §3 / `UPDATE 2026-09-24b` §3。

### 4. DoD 逐条核对

> 落点：`UPDATE 2026-09-24` §4 / `UPDATE 2026-09-24b` §4。

### 5. 偏差：none / DRIFT-0NN-x（全文按 gov §4.3 格式：现象 / 影响 / 建议 / 已停工作）

> 落点：`UPDATE 2026-09-23` §5 / `UPDATE 2026-09-23b` §5 / `UPDATE 2026-09-24` §5 / 本轮 `UPDATE 2026-09-24c` §5（DRIFT-011-1 / DRIFT-011-2）。

### 6. 更合理做法（非漂移，已直接落地 + 理由）

> 落点：`UPDATE 2026-09-23` §6 / `UPDATE 2026-09-23b` §6 / `UPDATE 2026-09-24` §6 / `UPDATE 2026-09-24b` §6 / 本轮 `UPDATE 2026-09-24c` §6。

### 7. 遗留问题（进 `docs/PARKING_LOT.md` 的编号）

> 落点：`UPDATE 2026-09-23c` §7 / `UPDATE 2026-09-24` §7 / `UPDATE 2026-09-24b` §7 / 本轮 `UPDATE 2026-09-24c` §7（**PL-053**）。

### 8. 新增长期记忆（FACT / PITFALL / REJECTED 条目原文；无则写"无"）

> 落点：`UPDATE 2026-09-23` §7 / `UPDATE 2026-09-23b` §8 / `UPDATE 2026-09-24` §8 / `UPDATE 2026-09-24b` §8。

### 9. 给审阅者的关注点（风险最高的 1~3 处）

> 落点：`UPDATE 2026-09-23` §9 / `UPDATE 2026-09-23b` §9 / `UPDATE 2026-09-24` §9 / `UPDATE 2026-09-24b` §9 / 本轮 `UPDATE 2026-09-24c` §9。
...
### UPDATE 2026-09-24c（治理回填：状态行 `Ready` → `Done` + §6 标题规范化）

用户指示（chat 2026-09-24）：「C 检查是否完整和正常，如果都正常，改为 Done，有错或者缺的部分补上，
然后 done。最后修改台账记录为正确的。」

#### §1 核对结论（本卡是否「完整和正常」）

| 检查项 | 结果 |
|---|---|
| 执行记录是否存在且完整 | ✅ 5 个 `UPDATE` 块（2026-09-23 / 23b / 23c / 24 / 24b）覆盖 §1~§11，含 DoD 逐条核对与验收输出 |
| `UPDATE 2026-09-24` §7 ①「必须由人类执行」的那一项 | ✅ **已闭合** —— `gate-selftest` run 2（`a3694ae`）与 run 3（`22e88a4`）均 SUCCESS（见 §10 / §11） |
| 9 节骨架标题齐全 | ⚠ **1 处不符**：§6 标题是「实施中发现的更合理做法」，而 `gov §3.4` 的规范标题是「更合理做法」 |
| 状态行 | ⚠ **不符**：卡内 `Ready`，而 `LEDGER.md` 自 2026-09-23 起有 10+ 行 `Done` 记录 |

→ 结论：**内容完整、结论成立**；两处「错/缺」已在下方 §5 记为 DRIFT 并当场修掉（只改标题与状态行）。

#### §5 偏差

**DRIFT-011-1（状态字段长期滞后）** —— **现象**：卡内 `状态：Ready` 与 `LEDGER.md` 的 `Done` 记录矛盾 ≥ 2 天，
期间 `plans/stage-1-pilots.md`（line 172）与 `PLAN.md` 都已按 Done 记载 → 三处事实源里唯一错的是**卡片自己**。
**影响**：下一会话读卡会误判「TASK-011 未开工」，可能重复派单（ADR-0031 D2 的失效模式）。
**建议**：按 ADR-0031 D2 把状态行改为 `Done（2026-09-24）`（已执行）；并把「卡状态 ↔ 台账」的机器一致性
校验登记为 **PL-053**（TASK-015 的 `check-ledger` 邻域）。**已停工作**：无（用户 chat 2026-09-24 已给出处置）。

**DRIFT-011-2（§6 标题偏离规范）** —— **现象**：§6 标题为「实施中发现的更合理做法」，`gov §3.4` 规范标题是
「更合理做法」。**影响**：状态改 Done 后 `card-check` 判据 ③ 会**新增 1 条 Warning**（`Ready` 状态被豁免，
所以此前一直没暴露）。**建议**：标题规范化为 `### 6. 更合理做法（非漂移，已直接落地 + 理由）`
（**只改标题行，正文一字未动**）—— 已执行。**已停工作**：无。

#### §6 更合理做法（本轮）

1. **空骨架节用「落点指引」而不是复制内容**：本卡内容已在 5 个 `UPDATE` 块里，逐节复制进骨架会造出
   **第二个事实源**（ADR-0030 的整条动机）。指引只写「哪一节去哪看」—— 一份内容一个落点。
2. **状态字段的机器校验必须存在**：本卡暴露的失效模式（卡内状态 vs 台账）靠人眼发现了 2 天，
   `card-check` 现有 5 条判据**一条都拦不住**它 → 归 TASK-015 的 `check-ledger`（见 §7）。

#### §7 遗留问题

- **PL-053（新提）**：卡片状态字段 ↔ `LEDGER.md` 的**机器一致性校验**（本卡 DRIFT-011-1 的根因）——
  归 TASK-015 的 `check-ledger` 或 ADR-0039 D3 邻域。
- 本卡原 §7 的其余项状态见 `UPDATE 2026-09-24` §7（① 已闭合；⑤ `envelope.error.code` 仍未修、需 ADR；
  `serde_json_lite::Value` 无大小上限归 TASK-012）。

#### §8 新增长期记忆

- 无新 FACT/PITFALL（状态回填是 routine 操作；本卡暴露的失效模式已作为 **DRIFT-011-1** 落盘 + **PL-053** 登记）。

#### §9 给审阅者的关注点

1. **只改了两处**：状态行（`Ready` → `Done（2026-09-24）`）与 §6 标题（规范化）。
   分界线以上的**卡片正文一字未动**（`git diff` 可逐行核对）。
2. **骨架 9 节的「落点指引」是新写的**：请确认它与各 `UPDATE` 块的实际节号对得上 ——
   尤其 **§8 指向 `UPDATE 2026-09-23` §7**，那一轮的 §7 标题就是「新增长期记忆」（节号在该轮内自编）。
3. **PL-053 的价值**：若认为「状态字段一致性」不值得机器化，请说明，我把它降级为人工核对项。
