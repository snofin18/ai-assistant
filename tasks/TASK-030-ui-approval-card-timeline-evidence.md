...
- 状态：**Ready**
- 阶段：1　子阶段：**1a**　批次：**A3**　依赖：029　预估：M　难度：M
...
- **依赖**：029　**预估**：M　**难度**：M
- **write scope**：`apps/desktop-ui/src/features/approval/**`、`.../timeline/**`
- **关联**：`plans/stage-1-pilots.md` 批次表 A3（1a）、`docs/wbs-overview.md` §6（DoD）

**目标**

UI：审批卡片（含 diff + 来源归因 + 授权范围）+ 执行时间线（含证据与撤销按钮）。

**write scope**（本卡独有部分，完整列表见 plan 批次表）

`apps/desktop-ui/src/features/approval/**`、`.../timeline/**`
