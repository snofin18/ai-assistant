...
- **依赖**：016　**预估**：M　**难度**：M
- **write scope**：`crates/platform/windows/src/input/**`、`.../src/coordinates/**`
- **关联**：`plans/stage-1-pilots.md` 批次表 A2（1a）、`docs/wbs-overview.md` §6（DoD）

**目标**

`platform/windows`：合成输入（SendInput）+ 焦点校验 + 坐标归一化（DPI/多屏）+ IME 处理。

**write scope**（本卡独有部分，完整列表见 plan 批次表）

`crates/platform/windows/src/input/**`、`.../src/coordinates/**`
