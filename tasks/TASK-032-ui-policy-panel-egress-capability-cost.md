...
- **依赖**：029,021　**预估**：M　**难度**：M
- **write scope**：`apps/desktop-ui/src/features/policy/**`、`.../capability/**`、`.../cost/**`
- **关联**：`plans/stage-1-pilots.md` 批次表 A3（1a）、`docs/wbs-overview.md` §6（DoD）

**目标**

UI：策略面板 + **出域三档开关**（含逐应用覆盖）+ Capability Matrix 视图 + 成本面板。

**write scope**（本卡独有部分，完整列表见 plan 批次表）

`apps/desktop-ui/src/features/policy/**`、`.../capability/**`、`.../cost/**`
