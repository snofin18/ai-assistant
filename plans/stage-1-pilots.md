...
> 周期 10~12 周　状态：**进行中**（stage-0 已 2026-09-20 closeout；1a 批次 **A1 全部 Done**：TASK-011 / 012 / 013 / 014 / **015**；A2 **TASK-016 已 Done**（`crates/platform/api` 落地）+ **TASK-017 已 Done**（`crates/platform/windows`：UIA provider 落地）→ 下一张 = **TASK-018**（`crates/platform/windows`：合成输入（SendInput）+ 焦点校验 + 坐标归一化 + IME 处理））　上位文件：`PLAN.md`
