...
```text
更新日期    ：2026-09-26（**PL-086 由 ADR-0047 裁决**：架构 v2 附录 A 的自由字符串 `assert` **永久不实现**，改为结构化 `field`/`op`/`value`；同批建三个**一次性** automation 跑 TASK-024 / 025 / 026 —— 00:00 / 03:00 / 06:00，`COUNT=1` 一手实测成立（`next_run_at` 触发后清空、仅 1 个 run）；上一张 Done 卡 = TASK-023 `verify` 后置断言引擎）
