...
```text
更新日期    ：2026-09-25（TASK-023 `verify` 后置断言引擎 Done：§7.4 的 11 种断言 + §7.3 状态指纹 + §8.5 幂等判定 + `on_violation` 分派；**验证失败绝不返回 ok** —— `Violated` / `Inconclusive` 都带 `VerifyFailed`；同批新增的 **PL-086 已由 ADR-0047 裁决：附录 A 的自由字符串 `assert` 永久不实现**）
