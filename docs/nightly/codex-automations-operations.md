...
> 状态：Active　版本：**1.7**　日期：**2026-09-25**　变更历史见 §9
...
> 机制**已可用**，但**当前 automation 数 = 0**（2026-09-25 建的两个已于当晚按人类指示**全部删除**；更早建的两个**常驻** automation 也已删除。**否证**：那两个任务原按「一次性」建立、实测却是**每天执行** —— `COUNT=1` 不被采纳，见 §4.1.a）
...
| 「一次性」的**具体写法** | ❌ **`[已否定]`（2026-09-25 当日闭环）** —— 候选「RRULE 加 `COUNT=1`」**不成立**：`rrule = "RRULE:FREQ=DAILY;COUNT=1;BYHOUR=13;BYMINUTE=30"` 能被工具接受并逐字落盘，但**调度器不采纳 `COUNT=1`** —— 两个任务跑完后**仍是 ACTIVE、未被消费**，且人类在 UI 看到的节奏是**每天**，即**事实上是每日重复任务**。**结论：用 RRULE 表达不了「一次性」**（另注意：应用**不校验**自己能否表达该 RRULE，会**静默降级**为每日 —— 这正是必须靠「跑完核对」而不是靠「创建成功」判定的原因）。**正确机制的候选（`[未验证]`）**：让 run 的**第一步就自删**（`automation_update(mode="delete", id=<自己的 id>)` —— 该动作本身在 2026-09-24 GATE-0.4 上实测可行），即「由任务自己保证只跑一次」，而不是指望 RRULE；见 **PL-081（已关闭）/ PL-084（新提）** |
...
> **当日闭环（2026-09-25 晚）**：人类核对 UI 后指出这两个任务**是每天执行**的 —— 即 `COUNT=1` **未被采纳**，「第二次触发不发生」**被否证**（不需要再等 2026-09-26）。两个任务已按人类指示**立即删除**（磁盘复核 `~/.codex/automations/` 只剩 `.run-jitter-salt`）。**结论**：一次性**不能**靠 RRULE 表达；下次建立必须改用「**run 自删**」或其他机制，且**每条都要当场核对磁盘 + 跑完后核对是否残留**（PL-084）。
