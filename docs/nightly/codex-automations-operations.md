...
> 状态：Active　版本：**1.10**　日期：**2026-09-26**　变更历史见 §9
...
> 建立需人类指令；**不同自动化之间间隔 ≥ 2 小时（默认 3 小时；ADR-0049 由 2.5 h 下调）**；**任务内容按实际进度现场决定**（prompt 不写死卡号）；
...
| 「不同自动化之间间隔 ≥ 2 h（默认 3 h）」 | ✅ 规则已定；**由人类给时刻时核对**，agent 只负责发现冲突并提示 |
...
1. 先看 .nightly.lock：不存在 → 建（写清 owner / 起始时间 / 本任务 id）；
   存在且 < 3 小时 → 视为别人在跑：本轮**不动代码、不提交**，只写 docs/nightly/<当夜日期>-skipped.md 后结束；
   存在且 ≥ 3 小时（陈旧）→ 把旧锁内容抄进本轮报告后再接管。
2. 顺序落盘：先建锁，再改仓库；**结束前必删自己的锁**（失败也要在报告里写明锁是否残留）。

【收尾 —— 轮次产物 + 晨间报告（章程 §11.3 / §11.4）】
3. 写 docs/nightly/<当夜日期>-round-<N>.md（N = 现有 round-* 文件数 + 1），记：本轮做了什么、依据哪些文件、验收结果、遗留；
4. 若本轮是当夜最后一次（人类指定，或按轮数 / 最后时刻判定）→ 另写 docs/nightly/<当夜日期>-report.md。
```
