...
> 状态：Active　版本：**1.11**　日期：**2026-09-26**　变更历史见 §9
...
#### 4.1.c 「prompt 固定块」：每轮必须写进 prompt 的两件事（`.nightly.lock` + 轮次产物）
...
但**没有一个**创建过 `.nightly.lock`，也**没有一个**产出 `docs/nightly/<当夜日期>-round-*.md` 或晨间报告 —— 因为**建任务时写进 prompt 的正文里没有这两步**。
章程 §11.2 把 `.nightly.lock` 定为**必需判据**，所以这是「章程 ↔ 实际 prompt」的缺口（登记 = `docs/PARKING_LOT.md` **PL-087**）。

**因此：每个 automation 的 prompt 必须含下列两个固定块（可照抄）**：

```text
【开工第一步 —— 仓库级互斥锁（章程 §11.2 / G8）】
1. 先看 .nightly.lock：不存在 → 建（写清 owner / 起始时间 / 本任务 id）；
   存在且 < 3 小时 → 视为别人在跑：本轮**不动代码、不提交**，只写 docs/nightly/<运行日日期>-skipped.md 后结束；
   存在且 ≥ 3 小时（陈旧）→ 把旧锁内容抄进本轮报告后再接管。
2. 顺序落盘：先建锁，再改仓库；**结束前必删自己的锁**（失败也要在报告里写明锁是否残留）。

【收尾 —— 轮次产物 + 晨间报告（章程 §11.3 / §11.4）】
3. 写 docs/nightly/<运行日日期>-round-<N>.md（N = 现有 round-* 文件数 + 1），记：本轮做了什么、依据哪些文件、验收结果、遗留；
4. 若本轮是当夜最后一次（人类指定，或按轮数 / 最后时刻判定）→ 另写 docs/nightly/<运行日日期>-report.md。
```

> **与 `#### 4.1.b` 自删的顺序**：自删仍是**整个 run 的最后一步**；`.nightly.lock` 的**释放**排在自删**之前**（先放锁，再自删）。
...
| 7 | 晨间报告落 `docs/nightly/<运行日日期>-report.md`（§11.4） | prompt 表达；需要沙箱允许写工作区 | ⚠ **worktree 模式下报告落在 worktree 里，主 checkout 看不到** → 人类早上打开仓库找不到报告。缓解：夜间 automation 用 **local 模式**，或规定人类从该 run 的 worktree/分支把报告取回 |
