...
> 状态：Active　版本：**1.5**　日期：**2026-09-25**　变更历史见 §9
...
> 机制**已可用**，但**当前 automation 数 = 0** —— 2026-09-25 建的两个**常驻** automation 已按人类指示**全部删除**
...
| 「一次性」的**具体写法** | ⚠ **`[未验证]`** —— 候选：RRULE 加 `COUNT=1`（如 `RRULE:FREQ=DAILY;COUNT=1`）或 `UNTIL`，或用 `DTSTART` + 单次规则；**工具是否有专用的一次性语义也未知** |
