...
> 状态：Active　版本：**1.8**　日期：**2026-09-25**　变更历史见 §9
...
| 「一次性」的**具体写法** | ✅ **`[已实测·已确认]`（2026-09-25 **深夜更正**；原先的 ❌ `[已否定]` 结论**被推翻**，见本小节末「更正块」）** —— 写法 = 「RRULE 加 `COUNT=1`」**成立**，**但必须同时给 `BYHOUR` 与 `BYMINUTE`**：`rrule = "RRULE:FREQ=DAILY;COUNT=1;BYHOUR=13;BYMINUTE=30"` 能被工具接受并逐字落盘，但**调度器不采纳 `COUNT=1`** —— 两个任务跑完后**仍是 ACTIVE、未被消费**，且人类在 UI 看到的节奏是**每天**，即**事实上是每日重复任务**。**结论（2026-09-25 深夜更正）**：`COUNT=1` + `BYHOUR` + `BYMINUTE` **就是**本项目的「一次性」写法（本机端到端实测：跑一次后 `next_run_at` 清空且不再重算）。**判据修正**：判定一次性要看 `next_run_at` 是否被清空 / 有没有第二个 run —— **不能**看 UI 的 Repeat 文案（UI 没有「单次」档，只会显示成「每日」），也**不能**看 `status`（跑完仍 `ACTIVE` 是设计如此）。**第二重保护（可选，非唯一机制）**：让 run 的第一步自删（`automation_update(mode="delete", id=<自己的 id>)`，该动作在 2026-09-24 GATE-0.4 上实测可行）；见 **PL-081（更正行）/ PL-084（更新）/ PL-085（新提）** |
