...
自动化路径是**两层锁**（章程 §11.2）：`.automation.lock`（仓库级，粗：自动化 vs 人工）＋ `guard`（文件级，细：
任意两个进程）。两层都要，不能只留一层 —— ①挡不住「两个人类会话同时改 `MEMORY.md`」，
②挡不住「自动化任务与人类同时改整个仓库」（guard 是文件级、协作式，且锁在 `target/` 下会被 `cargo clean` 删掉）。
