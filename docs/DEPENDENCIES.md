...
| `serde` | **`1.0`**（caret 语义：允许 1.x 内升级；`Cargo.toml` 与 `Cargo.lock` 实测一致） | `crates/protocol` / `crates/platform/api`（TASK-016） / `crates/ipc`（TASK-019） | serde derive + `serde_json::Value` for envelope.data / audit_event.target 等开放字段 | MIT OR Apache-2.0 | 替代 = 手写 Serialize/Deserialize impl = 出错率高且协议多版本难兼容 | **Approved** | 人类（chat 2026-09-23 TASK-103） | 2026-09-23 |
| `serde_json` | **`1.0`**（caret 语义：允许 1.x 内升级；`Cargo.lock` 实测解析到 1.0.151） | `crates/protocol` / `crates/audit` / `crates/ipc`（TASK-019） | JSON Value 类型（用于 envelope.data / error.details / metadata / audit_event.target 等开放字段）与 IPC 传输消息编码 | MIT OR Apache-2.0 | 替代 = serde_json::Value 是 serde_json 生态默认；不允许直接 String 传开放字段（违反 naming.md §5 newtype 约束）；IPC 需要同一份协议序列化路径 | **Approved** | 人类（chat 2026-09-23 TASK-103） | 2026-09-23 |
