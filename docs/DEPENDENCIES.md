...
| `serde` | **`1.0`**（caret 语义：允许 1.x 内升级；`Cargo.toml` 与 `Cargo.lock` 实测一致） | `crates/protocol` / `crates/platform/api`（TASK-016） / `crates/ipc`（TASK-019） / `crates/tool-bus`（TASK-020） | serde derive + `serde_json::Value` for envelope.data / audit_event.target 等开放字段 | MIT OR Apache-2.0 | 替代 = 手写 Serialize/Deserialize impl = 出错率高且协议多版本难兼容 | **Approved** | 人类（chat 2026-09-23 TASK-103） | 2026-09-23 |
...
| `sha2` | **`0.10`**（caret） | `crates/storage` / `crates/audit` / `crates/tool-bus`（TASK-020，工具集指纹） | 内容寻址的 SHA-256 摘要（blob_id 与完整性校验） | MIT OR Apache-2.0 | 替代 = `blake3`：更快，但 `sha256` 是**既定口径**（架构 v2 §15.3、审计 hash chain 同源），换算法 = 改契约 | **Approved** | 人类（chat 2026-09-24，TASK-012） | 2026-09-24 |
...
| `thiserror` | **`2`**（caret；`Cargo.lock` 实测 2.0.21） | `crates/tool-bus`（TASK-020） | `ToolBusError` 的 `derive(Error)`：14 个变体各自的 `Display` 文本（铁律 1 要求每个失败都带可读原因） | MIT OR Apache-2.0 | 替代 = 手写 `Display` + `std::error::Error`：14 个带结构化字段的变体，手写既冗长又容易漏分支；std 没有字段插值式错误派生的等价物。**已登记在案**：本 crate 是第一个使用方，`crates/core` 等的错误类型由各自任务卡按同一规则登记 | **Approved** | 人类（chat 2026-09-25 授权自决，TASK-020 Q5） | 2026-09-25 |
