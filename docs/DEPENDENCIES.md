...
| `serde` | **`1.0`**（caret 语义：允许 1.x 内升级；`Cargo.toml` 与 `Cargo.lock` 实测一致） | `crates/protocol` / `crates/platform/api`（TASK-016） / `crates/ipc`（TASK-019） / `crates/tool-bus`（TASK-020） / `crates/task-engine`（TASK-022） | serde derive + `serde_json::Value` for envelope.data / audit_event.target 等开放字段；**TASK-022**：检查点快照、Plan 与状态枚举的确定性序列化 | MIT OR Apache-2.0 | 替代 = 手写 Serialize/Deserialize impl = 出错率高且协议多版本难兼容 | **Approved** | 人类（chat 2026-09-23 TASK-103） | 2026-09-23 |
...
| `thiserror` | **`2`**（caret；`Cargo.lock` 实测 2.0.21） | `crates/tool-bus`（TASK-020） / `crates/task-engine`（TASK-022） | `ToolBusError` / `TaskEngineError` 用 `derive(Error)` 的 `Display` 文本（铁律 1 要求每个失败都带可读原因） | MIT OR Apache-2.0 | 替代 = 手写 `Display` + `std::error::Error`：带结构化字段的变体，手写既冗长又容易漏分支；std 没有字段插值式错误派生的等价物。**已登记在案**：`crates/core` 等的错误类型由各自任务卡按同一规则登记 | **Approved** | 人类（chat 2026-09-25 授权自决，TASK-020 Q5） | 2026-09-25 |
