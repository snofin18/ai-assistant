...
> 状态：Active　版本：**1.12**　日期：**2026-09-26**　变更历史见 §9
> 上位：**ADR-0029**（夜间自动化的投递机制改回 Codex 官方 scheduled tasks，Supersedes ADR-0018 的「Windows 任务计划程序 + `codex exec`」方案）、`docs/automation-charter.md` §11
...
> **适用范围（ADR-0050，2026-09-26）**：automation 可在**每天任何时间**运行，**不再限于夜间**。文中「当夜 / 晨间」为**历史命名**：「当夜」= **运行日**、「晨间报告」= **阶段报告**；路径 / 目录 / 锁名属历史命名（全量改名见 `docs/PARKING_LOT.md` PL-089）。
...
> 机制**已可用**，但**当前 automation 数 = 0**（2026-09-25 建的两个已于当晚按人类指示**全部删除**；更早建的两个**常驻** automation 也已删除。**更正（2026-09-25 深夜）**：那两个任务**确实是「一次性」** —— `COUNT=1` **被调度器采纳**，当时看到的「每天执行」来自 UI 把该规则显示成「每日」，见 §4.1.a 末「更正块」）
> **状态（2026-09-26 晨间复核）**：人类 2026-09-25 指令建立的三个一次性任务（`ai-assistant-one-shot-{0000,0300,0600}` → TASK-024 / 025 / 026）**已各触发一次并全部完成**（00:00:13 / 03:00:16 / 06:00:19，+08；三者 `next_run_at` 均已清空、无第二次触发），三张卡已合并进 `main`（PR #41~#44）。
...
| GATE-0.6 | **一主一备**：同一时刻只允许一种夜间机制处于 Active。启用回退方案（ADR-0018 的 CLI + 任务计划程序）之前，必须先暂停或删除 automation | 两套调度同时跑会抢 `.automation.lock`、产生无法归因的混合改动（章程 §11.2 的理由）。ADR-0029 D4 把 ADR-0018 定为**回退方案**，不是并行方案 |
...
**不通过怎么办**：回落到 ADR-0018 的路径（Windows 任务计划程序 + `codex exec`，验收清单见 `docs/automations/scheduler-acceptance-test.md`），或直接暂停夜间自动化、回到纯白天推进。ADR-0029 D4 已把 ADR-0018 定为「Superseded 但**保留**」的回退方案，那份清单不删除；启用它之前先按 GATE-0.6 停掉 automation。**不要试图改 rollout 来「修毒项」** —— 首轮投递的 `at_` id 从不落盘，无从修补 `[实测·探针]`。
...
| 「一次性」的**具体写法** | ✅ **`[已实测·已确认]`（2026-09-25 **深夜更正**；原先的 ❌ `[已否定]` 结论**被推翻**，见本小节末「更正块」）** —— 写法 = 「RRULE 加 `COUNT=1`」**成立**，**但必须同时给 `BYHOUR` 与 `BYMINUTE`**：`rrule = "RRULE:FREQ=DAILY;COUNT=1;BYHOUR=13;BYMINUTE=30"` 能被工具接受并逐字落盘，但**调度器不采纳 `COUNT=1`** —— 两个任务跑完后**仍是 ACTIVE、未被消费**，且人类在 UI 看到的节奏是**每天**，即**事实上是每日重复任务**。**结论（2026-09-25 深夜更正）**：`COUNT=1` + `BYHOUR` + `BYMINUTE` **就是**本项目的「一次性」写法（本机端到端实测：跑一次后 `next_run_at` 清空且不再重算）。**判据修正**：判定一次性要看 `next_run_at` 是否被清空 / 有没有第二个 run —— **不能**看 UI 的 Repeat 文案（UI 没有「单次」档，只会显示成「每日」），也**不能**看 `status`（跑完仍 `ACTIVE` 是设计如此）。**第二重保护 = 完成后自删**（配方见 **`#### 4.1.b`**）：让 run 的**最后一步**调 `automation_update(mode="delete", id=<本任务 id>)` 删掉自己（该动作在 2026-09-24 GATE-0.4 上实测可行）；相关登记 = **PL-081（更正行）/ PL-084 / PL-085** |
...
> **2026-09-25 深夜更正**：当时的判据（人类在 UI 看到「每天」+ 跑完仍 `ACTIVE`）**不足以否证**，结论见下面的**更正块**。两个任务已按人类指示删除（磁盘复核 `~/.codex/automations/` 只剩 `.run-jitter-salt`），**因果与当时记录相反**：它们是货真价实的一次性任务，只是 UI 会把它显示成「每日」。

> **更正块（2026-09-25 深夜；`[supersedes:2026-09-25]` 上面那个 ❌ `[已否定]` 与「当日闭环」段）**：
...
> **背景（2026-09-26 晨间复核）**：应用**没有**内置的「跑完即删」开关。asar 解包（`%LOCALAPPDATA%\Temp\asar_engine`，35 个文件）
...
> **不改变**「人类指令驱动 + 一次性」的形态；**也不**替代晨间人工/agent 复核（`next_run_at` 是否为 null、有无遗留条目）。

#### 4.1.c 「prompt 固定块」：每轮必须写进 prompt 的两件事（`.automation.lock` + 轮次产物）

> **依据 = 章程 §11.2（G8 两层互斥锁）/ §11.3（当夜与轮次产物）/ §11.4（晨间报告）；本子块由 ADR-0049 追加。**

**背景（2026-09-26 复核）**：2026-09-25 ~ 2026-09-26 实际跑完的 **7 个一次性 automation**（TASK-019 / 021 / 022 / 024 / 025 / 026 / 027）**全部成功**（各 1 turn），
但**没有一个**创建过 `.automation.lock`，也**没有一个**产出 `docs/automations/<当夜日期>-round-*.md` 或晨间报告 —— 因为**建任务时写进 prompt 的正文里没有这两步**。
...
【收尾 —— 轮次产物 + 晨间报告（章程 §11.3 / §11.4）】
3. 写 docs/automations/<运行日日期>-round-<N>.md（N = 现有 round-* 文件数 + 1），记：本轮做了什么、依据哪些文件、验收结果、遗留；
4. 若本轮是当夜最后一次（人类指定，或按轮数 / 最后时刻判定）→ 另写 docs/automations/<运行日日期>-report.md。
```
...
现行章程 = `docs/automation-charter.md` §11（**v1.9**，2026-09-25；机制 = Codex 原生 scheduled tasks；**排期形态 = ADR-0046 的「人类指令驱动的一次性任务」**）。ADR-0029 把机制从 ADR-0018 的 CLI 路径换回 Codex automation，下表据此逐条对表。下表的「机制缺口」= automation 表达不了、必须靠 prompt 纪律或人类审查兜住的部分。
...
| 2 | 每夜 ≤3 轮、每次运行 ≤2 轮（§11.3） | 轮次 N = `docs/automations/<当夜日期>-round-*.md` 文件数 + 1，由 agent 数文件；prompt 只**引用**章程、不复制规则 | ✅ 可表达（章程本来就是文件系统计数，与调度机制无关） |
...
| 4 | 只在 `automation/<date>` 分支提交（§0 底线 1） | **local 模式**：要 agent 自己 `git switch -c automation/<date>`（prompt 指令，无强制；官方警告 local 模式会改你正在编辑的文件 `[官方]`）。**worktree 模式**：隔离好，但分支名由 Codex 决定、能否指定 `[未验证]` | ❌ **机制缺口**：无法机器强制分支名。缓解：晨间报告 §① 必须抄 `git log` 的分支与提交清单，人类核对 |
...
| 7 | 晨间报告落 `docs/automations/<运行日日期>-report.md`（§11.4） | prompt 表达；需要沙箱允许写工作区 | ⚠ **worktree 模式下报告落在 worktree 里，主 checkout 看不到** → 人类早上打开仓库找不到报告。缓解：夜间 automation 用 **local 模式**，或规定人类从该 run 的 worktree/分支把报告取回 |
...
**小结**：14 条里 **5 条 ✅、5 条 ⚠、4 条 ❌**（#3 锁释放、#4 分支强制、#8 机器可读结果、#11 时长上限）。这 4 条是 ADR-0029 相对 ADR-0018 的**净损失**，必须由「prompt 纪律 + 晨间人工核对」承接；承接不了的就是 §7 里的 OPEN 项。作为交换，换回来的是：应用内可见性（Scheduled 收件箱与未读标记）、`notificationPolicy` 的可能性、以及「每次 run 天然新会话」。
...
| R10 | 无运行时长上限、无并发实例策略 | 一次卡死可能占住整夜和 `.automation.lock` | `[未验证]`（未见对应字段） | prompt 写时间预算 + 陈旧锁规则 + 早晨人工检查（§6 第 3、11 行） |
