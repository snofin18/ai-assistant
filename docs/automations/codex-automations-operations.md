...
> 状态：Active　版本：**1.14**　日期：**2026-09-26**　变更历史见 §9
> 上位：**ADR-0029**（自动化的投递机制改回 Codex 官方 scheduled tasks，Supersedes ADR-0018 的「Windows 任务计划程序 + `codex exec`」方案）、`docs/automation-charter.md` §11
...
> **适用范围（ADR-0050 / ADR-0052，2026-09-26）**：automation 可在**每天任何时间**运行（**不限于某个时段**）。全文一律用**时间中性**的说法：**自动化**、**运行日**、**阶段报告**；历史记录（§8.2 基线表、§9 变更历史、ADR / LEDGER / 任务卡）里的旧词保留不改。
> **改名（ADR-0051，2026-09-26）**：本文件所在目录由 **`docs/nightly/`** 改为 **`docs/automations/`**；正文里的章程路径 / 锁名 / 分支名同步换新（`docs/overnight-automation-charter.md` → `docs/automation-charter.md`、`.nightly.lock` → `.automation.lock`、`nightly/<date>` → `automation/<date>`）。**历史记录**（§8.2 基线表、§9 本表既有行、ADR / LEDGER / 任务卡）仍用旧名，不回溯改写。
...
> 机制**已可用**，但**当前 automation 数 = 0**（2026-09-25 建的两个已于当日按人类指示**全部删除**；更早建的两个**常驻** automation 也已删除。**更正（2026-09-25）**：那两个任务**确实是「一次性」** —— `COUNT=1` **被调度器采纳**，当时看到的「每天执行」来自 UI 把该规则显示成「每日」，见 §4.1.a 末「更正块」）
> **状态（2026-09-26 当日复核）**：人类 2026-09-25 指令建立的三个一次性任务（`ai-assistant-one-shot-{0000,0300,0600}` → TASK-024 / 025 / 026）**已各触发一次并全部完成**（00:00:13 / 03:00:16 / 06:00:19，+08；三者 `next_run_at` 均已清空、无第二次触发），三张卡已合并进 `main`（PR #41~#44）。
> ⚠ **跑完不会自动删除** —— 应用**没有**「跑完自删」内置选项（asar 全文检索 0 命中，见 §4.4 / §8.2），任务条目会一直留在 Scheduled 列表里（保持 `ACTIVE`，只是不再触发）→ 已由 agent 显式 `mode = "delete"` 清空，**当前 automation 数 = 0**（磁盘只剩 `.run-jitter-salt`）。**「让 run 自己删掉自己」的配方见 `#### 4.1.b`。**
> （磁盘复核：本机回到只剩 `.run-jitter-salt` 的基线）。人类同日给出的**正确形态**（**ADR-0046**）= **人类指令驱动的「一次性」任务**：
> 建立需人类指令；**不同自动化之间间隔 ≥ 2 小时（默认 2 小时）**；**同时存在的多个 automation 的开始时刻必须互斥**；**任务内容按实际进度现场决定**（prompt 不写死卡号）；
> **任何时间**均可提出。→ 「一次性任务」的建法已写进 §4.1 节末子块（`#### 4.1.a`）：**写法 = `RRULE` 加 `COUNT=1`，且必须同时给 `BYHOUR` / `BYMINUTE`（已本机端到端实测确认）**；跑完之后的清理走 `#### 4.1.b`（完成后自删）。见 §9 v1.2 ~ v1.9。
...
| GATE-0.2 | 兼容性验证**只允许挂在专门新建的废弃 thread 上**，该 thread 标题**必须以 `nightly-probe-` 开头**（挂 automation 前人工核对前缀；**不要**用 `mode = "view"` 判断，理由见 §4.8），且**两种形态都要验**：先用一次性 heartbeat 做最小验证，再用一个 standalone（cron）探针跑完整链路 | heartbeat 是最小可验证单位，但**生产形态是 standalone / cron**（ADR-0029 D1：heartbeat 不作为自动化主方案）。ADR-0018 记录两者的失败症状并不同（heartbeat 缺 `call_id`；cron 首轮生成 `at_<uuid>`）→ 只验一种会漏判 |
| GATE-0.3 | **绝不在主线工作 thread 上验证** | 主线 thread 一旦中毒，人类的交互一并废掉 —— 这是四个候选方案里唯一会污染主线工作的失效模式 |
...
| GATE-0.6 | **一主一备**：同一时刻只允许一种自动化机制处于 Active。启用回退方案（ADR-0018 的 CLI + 任务计划程序）之前，必须先暂停或删除 automation | 两套调度同时跑会抢 `.automation.lock`、产生无法归因的混合改动（章程 §11.2 的理由）。ADR-0029 D4 把 ADR-0018 定为**回退方案**，不是并行方案 |
...
**不通过怎么办**：回落到 ADR-0018 的路径（Windows 任务计划程序 + `codex exec`，验收清单见 `docs/automations/scheduler-acceptance-test.md`），或直接暂停自动化、回到纯人工推进。ADR-0029 D4 已把 ADR-0018 定为「Superseded 但**保留**」的回退方案，那份清单不删除；启用它之前先按 GATE-0.6 停掉 automation。**不要试图改 rollout 来「修毒项」** —— 首轮投递的 `at_` id 从不落盘，无从修补 `[实测·探针]`。
...
| 字段 | 说明与本项目取值建议 | 证据 |
|---|---|---|
| `name` | 人类可读、可 grep 的名字，例：`ai-assistant-automation-2330` | 必填 `[实测·探针]`（2026-09-17 探明，记录于 `docs/memory/pitfalls.md`） |
| `prompt` | 每次运行重放的指令。**用户可见**，写成连贯散文；**通知偏好不要写进来**（用 `notificationPolicy`） | 必填 `[实测·探针]`；写法要求 `[实测·声明]` |
| `rrule` | RFC 5545 递推规则。本项目「排期时刻（示例 23:30 / 02:30）」怎么写见 §6 第 12 行 | 字段必填 `[实测·探针]`；RRULE 语法 `[官方]` |
...
| 「一次性」的**具体写法** | ✅ **`[已实测·已确认]`（2026-09-25 **更正**；原先的 ❌ `[已否定]` 结论**被推翻**，见本小节末「更正块」）** —— 写法 = 「RRULE 加 `COUNT=1`」**成立**，**但必须同时给 `BYHOUR` 与 `BYMINUTE`**：`rrule = "RRULE:FREQ=DAILY;COUNT=1;BYHOUR=13;BYMINUTE=30"` 能被工具接受并逐字落盘，但**调度器不采纳 `COUNT=1`** —— 两个任务跑完后**仍是 ACTIVE、未被消费**，且人类在 UI 看到的节奏是**每天**，即**事实上是每日重复任务**。**结论（2026-09-25 更正）**：`COUNT=1` + `BYHOUR` + `BYMINUTE` **就是**本项目的「一次性」写法（本机端到端实测：跑一次后 `next_run_at` 清空且不再重算）。**判据修正**：判定一次性要看 `next_run_at` 是否被清空 / 有没有第二个 run —— **不能**看 UI 的 Repeat 文案（UI 没有「单次」档，只会显示成「每日」），也**不能**看 `status`（跑完仍 `ACTIVE` 是设计如此）。**第二重保护 = 完成后自删**（配方见 **`#### 4.1.b`**）：让 run 的**最后一步**调 `automation_update(mode="delete", id=<本任务 id>)` 删掉自己（该动作在 2026-09-24 GATE-0.4 上实测可行）；相关登记 = **PL-081（更正行）/ PL-084 / PL-085** |
| 「不同自动化之间间隔 ≥ 2 h（默认 2 h）；同时存在多个 automation 时开始时刻必须互斥（严禁同一时刻两个）」 | ✅ 规则已定；**由人类给时刻时核对**，agent 只负责发现冲突并提示 |
...
> **2026-09-25 更正**：当时的判据（人类在 UI 看到「每天」+ 跑完仍 `ACTIVE`）**不足以否证**，结论见下面的**更正块**。两个任务已按人类指示删除（磁盘复核 `~/.codex/automations/` 只剩 `.run-jitter-salt`），**因果与当时记录相反**：它们是货真价实的一次性任务，只是 UI 会把它显示成「每日」。

> **更正块（2026-09-25；`[supersedes:2026-09-25]` 上面那个 ❌ `[已否定]` 与「当日闭环」段）**：
> 上面两条结论**被推翻** —— `COUNT=1` **被调度器采纳**，「第二次触发不发生」**已被本机端到端实测确认**。
> **实测**（探针 `probe-count1-20260925`，`rrule = "FREQ=DAILY;INTERVAL=1;COUNT=1;BYHOUR=22;BYMINUTE=17"`，`ACTIVE`）：
> 建成时 `next_run_at = 2026-09-25 22:17:00` → **实际 22:17:11 触发** → 随后 `next_run_at` = **null**、
> `last_run_at = 22:17:11`、`status` 仍 `ACTIVE`；表 `automation_runs` **只有 1 行**、该 id 的 rollout **只有 1 个**。
> **为什么上次会误判**：① 应用 UI 的 Repeat 控件**没有「单次」档**（只有 hourly / daily / weekdays / weekly /
> custom / monthly / interval 七档），`COUNT=1` 的每日规则**只能显示成「每日」**；② 跑完仍 `ACTIVE` 是**设计如此**
> （一次性任务不转状态，只是把 `next_run_at` 清空）。**判据修正**：看 `next_run_at` 是否清空 / 有没有第二个 run，
> **不要**看 UI 的 Repeat 文案，也**不要**看 `status`（首次证据应取 `~/.codex/sqlite/codex-dev.db` 表
> `automations` 的 `next_run_at` / `last_run_at` 与表 `automation_runs` 的行数）。
> **两条硬性使用约束**：ⓐ `COUNT=1` **必须同时给 `BYHOUR` + `BYMINUTE`** —— 缺了它下次运行时刻直接算成 `null`
> （如 `FREQ=DAILY;COUNT=1` 会**永不触发且不报错**）；ⓑ **不要用 `COUNT≠1` 或 `UNTIL`** —— 它们会掉出「本地钟表」
> 快路径，`BYHOUR` 被按 **UTC** 解释（实测 `BYHOUR=9` → 17:00(+08)、`BYHOUR=14` → 22:00）。
> **仍未验证**：在 UI 里**编辑并保存**这样一条任务会不会把 `COUNT=1` 抹掉（表单重建 RRULE）→ **PL-085**；
> 在验证前，一次性任务**只在工具侧建立与删除**，不要在 UI 里改它的排期。
> 证据全文：`docs/memory/facts.md` 2026-09-25 的 `[supersedes]` 条、`docs/PARKING_LOT.md` PL-081 更正行。

#### 4.1.b 完成后自删（第二重保护）—— 让 run 自己删掉自己

> **背景（2026-09-26 当日复核）**：应用**没有**内置的「跑完即删」开关。asar 解包（`%LOCALAPPDATA%\Temp\asar_engine`，35 个文件）
> 全文检索 `deleteAfter` / `delete_after` / `autoDelete` / `oneShot` / `removeAfterRun` **0 命中**；调度引擎只暴露
> `onDeleteAutomation` 回调钩子（`src-DldfpmrL.js`）。→ 跑完的一次性任务**不会自己消失**：条目保持 `ACTIVE`、留在 Scheduled 列表里，
> 只有 `next_run_at` 被清空（= 不再触发）。**必须显式删除**，否则列表会逐夜堆积。

**配方**：建任务时把「**本 run 的最后一步 = 删除自己**」写进 prompt，并**把 id 直接写死在 prompt 里**：

```text
……（正文任务）……
最后一步：调用 automation_update(mode="delete", id="ai-assistant-one-shot-XXXX") 删除本任务自身；
然后核对 Get-ChildItem -Force -Recurse "$env:USERPROFILE\.codex\automations" 应回到只剩 .run-jitter-salt。
```
...
| 断言 | 强度 |
|---|---|
| run 能删掉自己（`mode = "delete"` 传自身 id 成功） | ✅ `[实测·探针]`（2026-09-24 GATE-0.4 的心跳 run 就是这样自删的；见 §4.4 / §8.2） |
| run 内**可知**自己的 id | ✅ `[实测·探针]` —— 投递的 user message 正文自带 `Automation ID:` 前缀（§8.2）；**且** id 是本项目建任务时自己选定的（由 `name` 派生），可直接写进 prompt，不依赖运行时读取 |
| 应用有「跑完自删」内置选项 | ✅ **无**（asar 全文检索 0 命中，见上） |
| **把自删放在第一步**是否影响本轮继续执行 | ❌ `[未验证]` —— GATE-0.4 那次自删发生在**最后**。**未验证前一律用「最后一步」** |

**为什么自删也要放在「最后一步」而不是「第一步」**：调度路径是 `Os()`（`nextRunAt <= now` 且是一次性 → `null`）
→ 持久化 `Pr(id, null)` → `await ks({...})` **才**真正开 run（`main-*.js`）—— 即 **`next_run_at` 在 run 启动之前就被清空**，
所以**端点故障 = 这一次直接丢失、不会重排**（没有失败重试自愈）。把自删放在最后一步，**失败的 run 就会留下任务条目** ——
这正好是**想要的**信号（遗留条目 = 需要人工看一眼）；放在第一步反而会在「任务其实没跑成」时把证据删掉。

> **仍必须遵守**：建立仍需人类指令（ADR-0046 A1）。本配方只把「跑完谁来删」从人工改成 run 自己，
> **不改变**「人类指令驱动 + 一次性」的形态；**也不**替代阶段人工/agent 复核（`next_run_at` 是否为 null、有无遗留条目）。

> **⚠ 自删的**前置条件**（ADR-0054 D1 / D2，2026-09-26）—— **先落地，再自删**：自删**之前**必须确认本轮的留痕已经在 `main` 上。
> 若本轮**产生了任何留痕**（任务卡执行记录 / `LEDGER.md` 行 / `docs/PARKING_LOT.md` 行 / `docs/memory/*` 条目 / 轮次产物 / 阶段报告）
> 却**没有**可合并的 PR → **必须先开一个 docs-only PR**（按 ADR-0046 A6 的判据合并）**再**自删。
> **唯一例外** = 本轮**完全没产生留痕**（判据 = `git status` 干净 **且** 本轮未写任何 `docs/**` / `tasks/**` / `LEDGER.md`）；
> §11.2 的 `-skipped.md` **本身就是留痕**，故**不**适用例外。**不确定时一律按「先落地」处理。**
> **反例（2026-09-26 真实发生）**：14:15 那次一次性自动化把 `DRIFT-206-1` 的全部证据提交在**未 push 的本地分支**
> 后就自删 → `main` 上既看不到卡片记录也看不到 `LEDGER.md` 行，下一个会话只能重新撞墙。

#### 4.1.c 「prompt 固定块」：每轮必须写进 prompt 的两件事（`.automation.lock` + 轮次产物）

> **依据 = 章程 §11.2（G8 两层互斥锁）/ §11.3（运行日与轮次产物）/ §11.4（阶段报告）/ **§11.12（先落地，再自删）**；本子块由 ADR-0049 追加，**ADR-0054** 修订。**

**背景（2026-09-26 复核）**：2026-09-25 ~ 2026-09-26 实际跑完的 **7 个一次性 automation**（TASK-019 / 021 / 022 / 024 / 025 / 026 / 027）**全部成功**（各 1 turn），
但**没有一个**创建过 `.automation.lock`，也**没有一个**产出 `docs/automations/<运行日日期>-round-*.md` 或阶段报告 —— 因为**建任务时写进 prompt 的正文里没有这两步**。
章程 §11.2 把 `.automation.lock` 定为**必需判据**，所以这是「章程 ↔ 实际 prompt」的缺口（登记 = `docs/PARKING_LOT.md` **PL-087**）。

**因此：每个 automation 的 prompt 必须含下列两个固定块（可照抄）**：

```text
【开工第一步 —— 仓库级互斥锁（章程 §11.2 / G8）】
1. 先看 .automation.lock：不存在 → 建（写清 owner / 起始时间 / 本任务 id）；
   存在且 < 3 小时 → 视为别人在跑：本轮**不动代码、不提交**，只写 docs/automations/<运行日日期>-skipped.md 后结束；
   存在且 ≥ 3 小时（陈旧）→ 把旧锁内容抄进本轮报告后再接管。
2. 顺序落盘：先建锁，再改仓库；**结束前必删自己的锁**（失败也要在报告里写明锁是否残留）。

【收尾 —— 轮次产物 + 阶段报告（章程 §11.3 / §11.4）】
3. 写 docs/automations/<运行日日期>-round-<N>.md（N = **main 上**现有 round-* 文件数 + 1 —— 先 `git fetch` 再 `git ls-tree --name-only origin/main -- docs/automations/`，**不要**数工作区；该路径已存在则**顺延**，**禁止覆盖**），记：本轮做了什么、依据哪些文件、验收结果、遗留；
4. 若本轮是运行日最后一次（人类指定，或按轮数 / 最后时刻判定）→ 另写 docs/automations/<运行日日期>-report.md。
```

> **与 `#### 4.1.b` 自删的顺序**：自删仍是**整个 run 的最后一步**；`.automation.lock` 的**释放**排在自删**之前**（先放锁，再自删）。
> **`[未验证]`**：把「自删」与「放锁」都放在最后一步是否会被中途失败打断 —— 失败时留下条目 + 残留锁正是「需要人工看一眼」的信号（与 §4.1.b 同一口径）。
...
② 自动化主方案**永久不用** heartbeat（章程 §11.1）—— 理由是**上下文累积漂移**，与 2026-09-24 已推翻的「中毒」不是同一件事，
所以「GATE-0 已通过」**不等于**「可以在主线 thread 上挂 heartbeat」。

**本项目定位（ADR-0029 D1）**：自动化主方案是 **standalone（cron）**；heartbeat **不作为自动化主方案** —— 它复用既有 chat 的上下文，一旦端点问题复现就是「毒项沉入长驻会话」的完整重演。heartbeat 在本项目只有一个合法用途：**GATE-0 的最小探针**（§2、§8 的 D1）。
...
**本项目纪律**：一次只改一个字段，改完立刻按 §4.1 的自检确认落盘，并在 LEDGER 追加一行（改排期属于「影响自动化行为」的变更，必须可归因）。
...
- **2026-09-26 复核** `[实测·探针]`：应用**没有**「跑完自删」内置选项（asar 全文检索 `deleteAfter` / `delete_after` / `autoDelete` / `oneShot` / `removeAfterRun` **0 命中**，只有 `onDeleteAutomation` 回调钩子）→ 跑完的一次性任务**不会自己消失**，**必须显式删除**。三个一次性任务（`-0000` / `-0300` / `-0600`）跑完后**确实**由 agent 逐个 `mode = "delete"` 清掉，`~/.codex/automations/` 与 `codex-dev.db` 的 `automations` / `automation_runs` 两表**双双回到 0 行**。让 run 自己删的配方见 `#### 4.1.b`。
...
- **本项目约定**：自动化 一旦连续两次运行 0 产出（章程 §11.5 的上报条件），**先暂停再排查**，不要边跑边改 —— 边跑边改会让你分不清「失败是旧配置造成的还是新配置造成的」。
...
现行章程 = `docs/automation-charter.md` §11（**v1.16**，2026-09-26；机制 = Codex 原生 scheduled tasks；**排期形态 = ADR-0046 的「人类指令驱动的一次性任务」**）。ADR-0029 把机制从 ADR-0018 的 CLI 路径换回 Codex automation，下表据此逐条对表。下表的「机制缺口」= automation 表达不了、必须靠 prompt 纪律或人类审查兜住的部分。

| # | 章程要求（出处） | 在 automation 机制下怎么表达 | 结论 |
|---|---|---|---|
| 1 | 「运行日日期」：00:00–11:59 开始的运行归属上一运行日（§11.3） | 机制没有「运行日」概念 → 把归属规则写进 prompt，由 agent 自己算日期 | ⚠ 可表达但脆弱：+2 分钟抖动（§5）会把贴近午夜的触发推过日界。**建议**计划时刻距 00:00 至少 10 分钟（ADR-0046 D5 起为建议、非硬规定） |
| 2 | 每次运行 ≤3 轮、每次运行 ≤2 轮（§11.3） | 轮次 N = `docs/automations/<运行日日期>-round-*.md` 文件数 + 1，由 agent 数文件；prompt 只**引用**章程、不复制规则 | ✅ 可表达（章程本来就是文件系统计数，与调度机制无关） |
| 3 | `.automation.lock` 互斥（§11.2 / 门禁 G8） | 只能由 prompt 指示 agent 获取与释放 | ❌ **机制缺口**：没有包装脚本的 `try/finally`，agent 中途失败或被 kill 时**锁不会释放**。缓解：保留章程的「3 小时陈旧锁」规则；获取锁优先用 ADR-0028 的 `xtask guard acquire`（自带等待/超时/放弃通报），但 guard 是**文件级**，仓库级互斥仍需 `.automation.lock` |
| 4 | 只在 `automation/<date>` 分支提交（§0 底线 1） | **local 模式**：要 agent 自己 `git switch -c automation/<date>`（prompt 指令，无强制；官方警告 local 模式会改你正在编辑的文件 `[官方]`）。**worktree 模式**：隔离好，但分支名由 Codex 决定、能否指定 `[未验证]` | ❌ **机制缺口**：无法机器强制分支名。缓解：阶段报告 §① 必须抄 `git log` 的分支与提交清单，人类核对 |
| 5 | 不合并 main（§0 底线 1） | prompt 禁令 | ❌ 无机器强制。而且官方说组织策略允许时 automation 用 `approval_policy = "never"` `[官方]` → 无人值守时**不会向任何人确认**。缓解：自动化白名单只留文档/测试类可逆工作（章程 §2），沙箱取最窄（官方也是这么建议的 `[官方]`） |
...
| 7 | 阶段报告落 `docs/automations/<运行日日期>-report.md`（§11.4） | prompt 表达；需要沙箱允许写工作区 | ⚠ **worktree 模式下报告落在 worktree 里，主 checkout 看不到** → 人类打开仓库找不到报告。缓解：自动化 用 **local 模式**，或规定人类从该 run 的 worktree/分支把报告取回 |
...
| 9 | 前置门禁 G1–G7（`cargo fmt` / `clippy` / `test` 全绿才开工，§1） | prompt 引用章程 §1，由 agent 自己跑 | ⚠ 沙箱坑：官方说 **workspace-write 下需要联网的工具调用会失败** `[官方]`。若 crates 未预先 fetch，`cargo test` 会因拉依赖而失败 → 自动化开工前必须**由人类先预热依赖**，或按官方说法用 rules 放行特定命令 `[官方]` |
| 10 | 运行前提：关机/睡眠不补跑（§11.7 ①） | 官方只要求「需要本地文件时保持电脑开机、应用运行，且项目在触发时刻仍在磁盘上」`[官方]` | ⚠ 错过之后是否补跑 `[未验证]`。章程的取舍是「跳过也无害」，两种行为都能接受，但要**实测记录**（§8 D6） |
| 11 | 硬超时 90 分钟（§11.7 ④、ADR-0018 风险表） | 没有对应字段 | ❌ **机制缺口**：无法设运行时长上限。缓解：prompt 写时间预算 + 章程 G7「当前时间 < 05:00」自检 + 人类确认没有卡死进程 |
| 12 | 排期形态：**人类指令驱动的一次性任务**（建立需人类指令；不同自动化之间**间隔 ≥ 2 h（默认 2 h）**；同时存在的多个 automation 开始时刻必须互斥；任务内容按实际进度现场决定）（ADR-0046 / 章程 §11.11） | 每个时刻**各建一个** automation，不用多时刻 RRULE（出问题时可以只暂停 / 只删除一个）；「一次性」的**具体写法** `[已实测·已确认]`（写法 = RRULE 加 `COUNT=1`，且必须同时给 `BYHOUR` / `BYMINUTE`，见 §4.1.a） | ✅ **一次性语义已实测**（本机端到端：计划时刻触发一次后 `next_run_at` 清空、无第二次触发；判据 = 不看 UI 文案、也不看 `status`）；`[未验证]` = RRULE 按哪个时区展开（本机 Asia/Shanghai） |
...
**小结**：14 条里 **5 条 ✅、5 条 ⚠、4 条 ❌**（#3 锁释放、#4 分支强制、#8 机器可读结果、#11 时长上限）。这 4 条是 ADR-0029 相对 ADR-0018 的**净损失**，必须由「prompt 纪律 + 阶段人工核对」承接；承接不了的就是 §7 里的 OPEN 项。作为交换，换回来的是：应用内可见性（Scheduled 收件箱与未读标记）、`notificationPolicy` 的可能性、以及「每次 run 天然新会话」。
...
| R2 | 触发有随机抖动（2026-09-17 记 ≈ +2 分钟；**2026-09-24 复测 cron +46 s、heartbeat 约 +1 s**） | 定时不准；贴近午夜会把「运行日日期」算错 | `[实测·探针]` | 计划时刻避开 00:00 与整点边界至少 10 分钟；日期归属规则写进 prompt（§6 第 1 行） |
...
| R6 | worktree 堆积 | 磁盘占用与分支噪音 | `[官方]` | 自动化用 local 模式，或按官方要求定期归档不再需要的 run、且不 pin run |
| R7 | heartbeat 污染长驻 thread | 主线工作会话永久报废，人类交互一并受牵连 | `[实测·探针]` | 主线 thread 永久禁挂；验证只用废弃 probe thread（GATE-0.2 / GATE-0.3） |
| R8 | `notificationPolicy` 与 `failed_runs_only` 官方未文档化 | 「仅失败时通知」可能不成立 → 要么每次运行两条噪音通知，要么反过来失败也不通知 | `[实测·声明]`；官方页无此词 `[官方未写]` | §8 D4 实测；不成立就退回「只有文件、没有推送」（章程 §11.6 的现状） |
| R9 | `update` 是全量替换语义 | 漏一个字段就可能抹掉既有配置 | `[实测·声明]` | 改前先抄 `automation.toml` 全量字段；一次只改一个字段并立刻自检 |
| R10 | 无运行时长上限、无并发实例策略 | 一次卡死可能占住本次运行和 `.automation.lock` | `[未验证]`（未见对应字段） | prompt 写时间预算 + 陈旧锁规则 + 人工检查（§6 第 3、11 行） |
| R11 | 组织策略允许时 automation 用 `approval_policy = "never"` | 无人值守时不会向任何人确认，与 AGENTS.md 铁律 6 直接冲突 | `[官方]` | 自动化白名单只留可逆的文档/测试类工作；沙箱取最窄；真实应用与资金/发送类永久排除 |
...
| R15 | **沙箱开关不在 `config.toml`**：应用 UI 的权限模式（`~/.codex/.codex-global-state.json` → `permission-selection-by-host-id:local`）覆盖 `sandbox_mode` | 以为改了 `config.toml` 就收紧了沙箱 → **实际仍是 full access**（静默地不安全）；且 automation 用「你的默认沙箱设置」，所以自动化也照此 | `[实测·探针]`（2026-09-24：改 `config.toml` 后新建的 run 仍报 `sandbox=dangerFullAccess`，工作区外写入成功） | **收紧沙箱必须去应用 UI 改权限模式**（config.toml 不够）；改完用「工作区外写入应失败」做**负向验证**。另注：`workspace-write` 下需要联网的工具调用会失败（§6 第 9 行）→ 先 `cargo fetch` 预热 |

---

## 8. 验证清单（**2026-09-24 已执行** —— 结论见 §8.2 / §8.3）

> 断言对象换成 Codex automation；格式沿用 `docs/automations/scheduler-acceptance-test.md`。按 ADR-0029 D4，那份清单**不删除**，改挂「已被取代（当前非主方案），保留为回退方案验收清单」横幅 —— 该标注由主会话执行，**不在本手册的 write scope 内**。
...
| P5 | 工作区干净、无残留锁 | `git status --porcelain`；`Test-Path .automation.lock` | 空输出；`False` |
...
| 应用是否有「跑完自删」内置选项 | **无** —— asar 解包（35 文件）全文检索 `deleteAfter` / `delete_after` / `autoDelete` / `oneShot` / `removeAfterRun` **0 命中**；调度引擎只有 `onDeleteAutomation` 回调钩子（`src-DldfpmrL.js`） | 2026-09-26 |
| 「一次性」是不是调度引擎的一等概念 | **是** —— `jB(rrule)` 解析 RRULE 后判定「恰好 1 条 rrule + `count === 1` + 无 rdate / exrule / exdate」（`src-DldfpmrL.js`） | 2026-09-26 |
| `next_run_at` 与 run 的先后顺序 | **先清空、后启动** —— `Os()`（`nextRunAt <= now` 且是一次性 → `null`）→ 持久化 `Pr(id, null)` → `await ks({...})` 才开 run（`main-*.js`）→ 端点故障时**这一次直接丢失、不重排**，任务条目留在列表里 | 2026-09-26 |
| 跑完的一次性任务会不会自己消失 | **不会** —— 条目保持 `ACTIVE`、留在 Scheduled 列表，只有 `next_run_at` 变 `null` | 2026-09-26 |
| 三个一次性任务（`-0000` / `-0300` / `-0600`）实跑 | 各触发**一次**并全部完成：00:00:13 / 03:00:16 / 06:00:19（+08）；三者 `next_run_at` 均清空、`automation_runs` 各 1 行、无第二次触发；跑完由 agent 显式删除 | 2026-09-26 |

### 8.3 判定与后续

- **全绿** → ① 把 §2 的 GATE-0 标为已通过 + 日期 + 执行人；② ADR-0029 的「生效前提」标为已满足；③ 章程 §11 按 §6 的映射表重写（改章程需 ADR，ADR-0029 即该授权）；④ 才可创建**正式**的自动化排期；⑤ `LEDGER.md` 追加一行；⑥ 把 D2–D6 的实测结论回填本手册，替换相应的 `[未验证]` 标签。
- **任一项失败** → 不建正式排期；失败项与证据记入 `docs/PARKING_LOT.md`；判断是回落到 ADR-0018 的 CLI 路径，还是触发「连续失败即暂停自动化、回到纯人工推进」的同类条款。
...
| 1.7 | 2026-09-25 | **「一次性」写法实测被否定 + 状态行同步**：§4.1.a 的「「一次性」的具体写法」由 `[部分验证]` 改为 ❌ **`[已否定]`** —— `COUNT=1` 被工具接受并逐字落盘、但**调度器不采纳**（两个任务跑完仍 ACTIVE 未被消费 + 人类 UI 观察为「每天」），并写明「应用会**静默降级**为每日，所以判据必须是跑完核对而非创建成功」，给出候选替代机制（**run 第一步自删**，`[未验证]`）；§4.1.a 末尾的「诚实边界」段落改写为**当日闭环**结论；§2 状态行的「automation 数 = 2」改为「**= 0**（两个已删）」；文件头版本 1.6 → **1.7**。§1 / §3 / §4.0~§4.8 其余 / §5~§8 一字未改 | 人类 chat 2026-09-25「两个自动化你建成了非一次性的，是每天执行的了。这违反了当初的规定」；`automation_update(mode=delete)` × 2 + 磁盘复核；**ADR-0046 D2**（验证判据）+ **ADR-0041 D5** |
| 1.8 | 2026-09-25 | **「一次性」写法更正（结论反转）**：§4.1.a 的「具体写法」由 ❌ `[已否定]` 改为 ✅ **`[已实测·已确认]`** —— `RRULE` 加 `COUNT=1` + `BYHOUR`/`BYMINUTE` **就是**可用的「一次性」写法（本机端到端：计划 22:17:00 → 22:17:11 触发一次 → `next_run_at` 清空、`automation_runs` 仅 1 行）；§4.1.a 末尾新增**更正块**（含「为什么上次误判」= UI 无单次档 + 跑完仍 ACTIVE 是设计如此，判据修正，以及两条硬性使用约束）；§2 状态行的「否证」段与 §4.1.a 的「当日闭环」段同步改写；文件头版本 1.7 → **1.8**。§1 / §3 / §4.0~§4.8 其余 / §5~§8 一字未改 | 2026-09-25 深夜本机端到端实测（`probe-count1-20260925` → `~/.codex/sqlite/codex-dev.db` 的 `automations` / `automation_runs`）+ 应用自带调度引擎源码（asar 解包）；**PL-081 更正行 / PL-084 更新 / PL-085 新提** |
| 1.9 | 2026-09-26 | **新增 `#### 4.1.b`「完成后自删（第二重保护）」配方 + §4.4 补实测 + §8.2 追加 5 行基线 + §2 状态行同步**：把 2026-09-25 深夜只当「可选机制」提过的「run 自删」写成可照做的配方（自删写进 prompt 的**最后一步** + **id 直接写死**），并标清证据强度（run 能删自己 = `[实测·探针]`；把自删放**第一步**是否影响本轮执行 = `[未验证]`）；§4.4 补「应用**没有**内置跑完自删开关（asar 全文检索 0 命中）→ 必须显式删除」；§8.2 追加 5 行（无内置自删 / 一次性是一等概念 / `next_run_at` 先清空后启动 / 跑完不自动消失 / 三个一次性任务实跑记录）；§2 状态行补 2026-09-26 晨间复核块，并把 2026-09-24 状态行里过期的「具体写法 `[未验证]`」改成已实测口径；§4.1.a 只改「第二重保护」那一句（指向 `#### 4.1.b`）；文件头版本 1.8 → **1.9**。§1 / §3 / §4.0 / §4.2 / §4.3 / §4.5~§4.7 / §5~§7 一字未改 | 人类 chat 2026-09-26「三个自动化还是没有删除。需要完成后及时删除。找找有什么办法可以实现完成后自动删除」；本机实测（`codex-dev.db` 的 `automations` / `automation_runs` 双双 0 行 + `~/.codex/automations/` 只剩 `.run-jitter-salt`）+ asar 解包源码；**ADR-0046 D2**（验证判据回填）；**PL-084** |
| 1.10 | 2026-09-26 | **最小间隔 2.5 h → 2 h（ADR-0049）+ 新增 `#### 4.1.c`「prompt 固定块」**：§4.1 顶部引文、§4.1.a 的规则行、§6 映射表第 12 行的数值由 2.5 h 改为 **2 h（默认 3 h 不变）**，并**修正该行两处过期标记**（`[未验证]` → `[已实测·已确认]`、`⚠ 机制未验证` → `✅ 一次性语义已实测`）+ 同步 §6 小结计数 4✅/6⚠/4❌ → **5✅/5⚠/4❌**；**新增 §4.1.c**，把章程 §11.2 的 `.nightly.lock` 获取 / 释放与 §11.3 / §11.4 的 `docs/nightly/<当夜日期>-round-*.md` / 晨间报告写成**可照抄的 prompt 固定块** —— 起因 = 2026-09-26 复核发现 7 个一次性 run **全部成功但无一建锁、无一写轮次产物**（prompt 正文里没有这两步；登记 PL-087）；并写明与 `#### 4.1.b` 自删的顺序（先放锁、再自删）；文件头版本 1.9 → **1.10**。§1 / §3 / §4.0 / §4.2~§4.8 / §5~§8 正文一字未改 | 人类 chat 2026-09-26「如果运行时间小于 2.5 小时，那么要给一个新的最小运行间隔时间」+「检查自动化整个流程是否运转正常」；7 个一次性 run 的一手时长与「无锁 / 无轮次产物」复核；**ADR-0049**（授权修订 §4.1 并新增 §4.1.c）；**PL-087** |
| 1.11 | 2026-09-26 | **时间中性化 + 默认间隔 2 h + 开始时刻互斥（ADR-0050）**：① 文件头新增**适用范围**声明（automation 适用于**每天任何时间**；「当夜」= **运行日**、「晨间报告」= **阶段报告**；路径 / 目录 / 锁名属历史命名 → PL-089）；② §4.1 顶部引文、§4.1.a 的规则行、§6 映射表第 12 行的**默认值 3 h → 2 h**（下限仍 2 h），并写入**开始时刻互斥**（同时存在 ≥ 2 个 automation 时严禁同一时刻开始 / 同时运行）；③ §4.1.c 的产物名由 `<当夜日期>` 改为 `<运行日日期>`；文件头版本 1.10 → **1.11**。§1 / §3 / §4.0 / §4.1.b / §4.2~§4.8 / §5 / §7 / §8 正文一字未改 | 人类 chat 2026-09-26 三点指令；**ADR-0050**（授权修订 §4.1 / §4.1.a / §4.1.c / §6） |
| 1.12 | 2026-09-26 | **去夜间化改名（ADR-0051）**：目录 `docs/nightly/` → **`docs/automations/`**；正文里章程路径 / 锁名 / 分支名换新；文件头加改名注记。§8.2 基线表、§9 本表既有行保留旧名，不回改 | 人类 2026-09-26「PL-089 可以改名」；**ADR-0051** |
| 1.13 | 2026-09-26 | **全量措辞去夜间化（ADR-0052）**：§1~§8 的 `夜间 / 每夜 / 当夜 / 晨间 / 整夜` 全量换成 `自动化 / 每次运行 / 运行日 / 阶段报告 / 本次运行`；文件头适用范围声明改为「一律时间中性」。§8.2 基线表、§9 变更历史既有行、数字预算不动 | 人类 2026-09-26「做「全量措辞」扫」；**ADR-0052** |
| 1.14 | 2026-09-26 | **「先落地，再自删」+ 轮次编号取 `main`（ADR-0054）**：§4.1.b 新增自删的**前置条件**（本轮产生留痕却没可合并 PR → 必须先开 docs-only PR 再自删；唯一例外 = 完全没产生留痕；`-skipped.md` 不属例外）；§4.1.c 的固定块把轮次编号的取数改为 **`main`**（`git ls-tree`，禁止数工作区；路径已占用则顺延、禁止覆盖）；§4.1.c 依据行补 §11.12 | 2026-09-26 两个一次性自动化暴露：① 14:15 轮把 DRIFT-206-1 证据留在未 push 的本地分支后自删；② 同日两轮都写 `round-1`，前一轮记录被覆盖 |
