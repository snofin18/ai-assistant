...
- **单会话上限**：一个会话最多完成 **1~2 张任务卡**。做完就提交、**同步 `PLAN.md` 当前状态块 + `README.md` 三处
  （状态行 / `## 当前阶段` / `## 最近进展`）**、更新 LEDGER、结束会话。
  > **为什么卡 Done 就必须同步这两份文件**（**ADR-0039**）：`PLAN.md` 是 `AGENTS.md` §3 启动第 ② 步的入口，
  `README.md` 是对外状态；把「同步」绑到「卡 Done」上，是因为**做事的那个人**才刚知道真实状态。
  `PLAN.md` 除「当前状态」块外的段落仍归 Orchestrator。
...
- [ ] 无 / 有（DRIFT-编号 + 处理结果）
## 文档同步
- [ ] `PLAN.md` 的「当前状态」块 4 行已更新（**每张卡 Done 都要**，ADR-0039 D1 / D2）
- [ ] `README.md` 三处已更新（状态行 / `## 当前阶段` / `## 最近进展`；无变化也要追加，ADR-0039 D1 / D2）
