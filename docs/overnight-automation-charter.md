...
> 状态：Active　版本：**1.13**　日期：**2026-09-26**　变更历史见 §12
...
- **排期形态 = 人类指令驱动的一次性任务（ADR-0046）**：每次由人类下指令时建立；**不同自动化之间
  间隔 ≥ 2 小时（默认 3 小时）**；**不得**建立常驻的每日 / 每周重复任务，**不得**由 agent
...
| **A2** | **自动化都是一次性的**；**每个不同自动化之间的间隔至少 2 小时，默认 3 小时**（数值经 **ADR-0049** 由 2.5 h 下调） | 人类点 2 |
