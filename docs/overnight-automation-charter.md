...
> 状态：Active　版本：**1.7**　日期：**2026-09-25**　变更历史见 §12
...
| 地位 | 机制 | 依据 | 现状 |
|---|---|---|---|
| **主方案** | **Codex 原生 scheduled tasks**，形态 = **standalone（cron 类）** | ADR-0029 D1 | ✅ **已可用**：启用前置门禁 GATE-0 于 **2026-09-24** 通过（§11.9）；**已排期（2026-09-25）**：**两个**单时刻 automation（§11.3 的「建两个」口径）—— `ai-assistant-nightly-task-chain`（每日 **02:30**，当夜最后一次，兼写当夜报告）与 `ai-assistant-nightly-task-chain-2330`（每日 **23:30**，当夜第一次）；均 `kind=cron` / `executionEnvironment=local` / `notificationPolicy=failed_runs_only` |
...
- **为什么不许双轨并行**（ADR-0029 选项 4）：「两个都是主方案」= 没有主方案。锁语义、报告位置、
  ...
  （操作见操作手册 §4.6 / §4.4）。
- **每夜两次**（23:30 与 02:30）建议**建两个单时刻的 automation**，而不是一条 `BYHOUR=23,2` 的 RRULE：
  出问题时可以只暂停一个（操作手册 §6 第 12 行）。计划时刻**距 00:00 至少 10 分钟** ——
  automation 有 +2 分钟随机抖动（来自 `~/.codex/automations/.run-jitter-salt`），
  贴近日界的触发会被推过边界而算错「当夜」（§11.3）。
- **沙箱取最窄**：与官方建议一致（「从能让任务成功的最窄访问开始，只在必要时才给网络或更大文件访问」
  `[官方]`）。夜间白名单本来就只有文档/测试类可逆工作（§2），不需要网络（§11.7 ④）。

### 11.2 两层互斥锁（G8）—— **保留，并按 ADR-0028 升级为两层**

夜间任务与人类会话**可能同时存在**；两个 agent 同时改同一仓库会产生无法归因的混合改动，
这比「少做一夜」严重得多。automation 机制**没有包装脚本的 `try/finally`**，所以锁纪律必须写进 prompt：

| 层 | 载体 | 粒度 | 谁负责 | 挡住什么 |
|---|---|---|---|---|
| ① 仓库级 | `<repo>/.nightly.lock`（已在 `.gitignore`，**不得**提交） | 整个仓库 | agent 按 prompt 指示自己获取/释放 | 「夜间任务 vs 白天人类会话」同时改仓库 |
| ② 文件级 | `cargo run -p xtask -- guard acquire <路径> --owner nightly-<当夜日期>`（ADR-0028） | 单个文件 | agent 改写任何公共热点文件**前**必取 | 两个进程改同一文件（`LEDGER.md` / `docs/memory/*.md` / `docs/PARKING_LOT.md` 是最高危的三个只追加热点） |
...
- ⚠ automation 机制**没有「当夜」概念**，归属只能由 agent 按上面的规则自己算（操作手册 §6 第 1 行）；
  prompt 只**引用**本节，不复制规则。抖动会把贴近午夜的触发推过日界 → 故 §11.1 规定计划时刻
  距 00:00 至少 10 分钟。

### 11.4 谁写晨间报告 —— **保留，内容要求按新机制调整**
...
→ 机制**已可用**；**已排期（2026-09-25）** —— 人类 2026-09-25 授权后已创建正式 automation（见 §12 v1.6 / v1.7）。**频率 = 每夜两次（23:30 + 02:30）**，与 §11.3 一致。首夜（09-25 凌晨）只建了 02:30 一次（偏差登记为 PL-076），已于同日补建 23:30 那一次、并把 §11.2 / G8 的 `.nightly.lock` 协议写进两份 prompt（**PL-076 关闭**）。
