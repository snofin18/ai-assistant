...
> 状态：Active　版本：**1.4**　日期：**2026-09-18**　变更历史见 §12
...
| 地位 | 机制 | 依据 | 现状 |
|---|---|---|---|
| **主方案** | **Codex 原生 scheduled tasks**，形态 = **standalone（cron 类）** | ADR-0029 D1 | ⏳ **未启用**：启用前置门禁 GATE-0 未通过（§11.9） |
...
**当前状态（2026-09-18）：⏳ GATE-0 未执行。** 本轮（ADR-0029 D2）**只调研落档，不做端到端调试**，
也**没有创建、修改或删除任何真实 automation**（探针只用「缺字段报错」的方式安全探明 schema）。
→ 在此之前**不得**创建指向真实工作 thread / 项目的 automation；调试需人类专门安排时段
（`docs/memory/open.md` **N9**）。
