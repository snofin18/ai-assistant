...
> 状态：Active　版本：**1.8**　日期：**2026-09-25**　变更历史见 §12
...
| 地位 | 机制 | 依据 | 现状 |
|---|---|---|---|
| **主方案** | **Codex 原生 scheduled tasks**，形态 = **standalone（cron 类）** | ADR-0029 D1 | ✅ **已可用**：启用前置门禁 GATE-0 于 **2026-09-24** 通过（§11.9）；**当前未排期**（2026-09-25 已按人类指示**删除全部** automation）：人类同日澄清**正确形态** = 「**由人类每晚上手动下指令时建立一次性任务**；一晚 **3 个**、每次**间隔 3 小时**（杜绝两个同时运行）；**任务内容按实际进度现场决定**，不预先绑定具体卡号」→ §11.3 的常驻「每夜两次」口径**需要修订**（属章程内容变更 = 需 ADR）。见 `docs/PARKING_LOT.md` **PL-077** |
...
→ 机制**已可用**，但**当前未排期**：本轮建的 2 个**常驻** automation 已于 2026-09-25 按人类指示**全部删除** —— 已按手册 §4.8 用磁盘复核，`~/.codex/automations/` 回到只剩 `.run-jitter-salt` 的基线。人类同日澄清**正确形态** = 「**由人类每晚上手动下指令时建立一次性任务**；一晚 3 个、间隔 3 小时、按实际进度决定下一张卡」⇒ §11.3 的常驻「每夜两次」口径**需修订**（改章程内容 = 需 ADR）。详见 §12 v1.8 与 `docs/PARKING_LOT.md` **PL-077**。
