...
- [2026-09-18][OPEN][N4] **夜间自动化的验收测试尚未真跑过**（人类指示 #10 要求"专门留个时间测试"）。断言清单已写在 `docs/nightly/scheduler-acceptance-test.md`，但 `Register-ScheduledTask` + `codex exec` 的端到端一次成功运行**还没有本机证据**。→ 需人类在场时跑一次冒烟；在此之前章程 §11 处于"设计已定、未验收"状态。
  - [supersedes:2026-09-24] → **已由 N9 / GATE-0 承接，且已通过（2026-09-24）**。本条原指「`Register-ScheduledTask` + `codex exec` 端到端一次成功运行」—— 那属**回退方案**（ADR-0018，已被 ADR-0029 降级为 Superseded-but-available），**主方案不再需要它**。`docs/nightly/scheduler-acceptance-test.md` 保留为回退方案清单，**仅当回退方案启用时**才跑。**本条视为关闭（无待办）**。
- [2026-09-18][OPEN][N5] **`codex exec` 的退出码语义无官方文档**。章程与验收脚本只能**自建基线**（首次冒烟时把观察到的退出码记录下来当作事实源）。→ 归 N4 一并做。
  - [supersedes:2026-09-24] → **仅属回退方案，主方案挂起**：`codex exec` 退出码基线只在启用 ADR-0018 的 CLI 路径时才需要；主方案（Codex automation）**根本没有退出码**（章程 §11.10 第 8 行：无机器可读的成功/失败判据）。**本条视为挂起**（回退方案启用时再开）。
...
- [2026-09-18][OPEN][N9] **夜间自动化的启用前置门禁 GATE-0 尚未执行**（ADR-0029 D3；本条**承接** N4/N5 的验收对象）。需要实测的全链路：Codex automation 触发 → 本机第三方端点（阿里云百炼 compatible-mode，`qwen3.8-max`）接受那条 `call_id: None` 的合成 `function_call_output` → 模型正常回复 → 产物落盘。**必须在标题以 `nightly-probe-` 开头的新建废弃 thread 上做**，绝不在主线工作 thread 上（ADR-0018 的 E3 已证明毒项 + 全量重放会**永久损坏**长驻会话）。**本轮刻意未执行** —— 人类指示 #2 是「查清楚官方文档的具体写法，到时候专门找时间调试」，故 ADR-0029 D2 把本轮范围限定为「只调研落档」。→ 在此之前**禁止创建任何真实 automation**；断言清单在操作手册 §8，门禁表在 §2。
  ...
  - [supersedes:2026-09-24] → **已关闭：GATE-0 于 2026-09-24 实测通过**。GATE-0.1~0.4 四条全部成立：cron（standalone）与 heartbeat **两种形态**都验过；探针只在标题以 `nightly-probe-` 开头的**一次性废弃 thread** 上跑、**未触碰主线工作 thread**；探针 automation 已删除、`~/.codex/automations/` 已回到只剩 `.run-jitter-salt`。**关键新事实**：automation 的投递形态已变为**正常 user message** → 原 E1 的 400 根因消失、「毒项沉入长驻会话」未复现（`call_id` 全合法、`at_` id 0 个）。证据 = `docs/nightly/codex-automations-operations.md` §2 / §8.2（18 行新基线）+ `docs/memory/facts.md` / `pitfalls.md` 的 2026-09-24 条目。→ 机制**已可用**；但**尚未排期**（人类 2026-09-24 指示「何时排期另行说明」）。**本条关闭。**
