...
## 2026-09-24 追加（ADR-0043 / ADR-0044 / ADR-0045，Accepted）

- [2026-09-24][DECISION][src:ADR-0043] **元素解析必须有 scope（父窗口）**：`UiAutomationProvider` 的 `resolve_element` / `wait_for` 首参加 `&ResolvedWindow`，搜索起点 = 该窗口的 UIA 根元素（`ElementFromHandle`），**禁止**从桌面根搜元素。**根因** = PL-068：TASK-016 冻结的 trait 形状没有 scope 参数 → TASK-017 只能对桌面根发 `FindAll(TreeScope_Descendants)`，2026-09-24 真机实测**中位数 1.53 s**（对照窗口子树内 1.2~1.5 ms，约 **1000×**），且违反 ADR-0022 E6 引用的官方要求（桌面上找顶层窗口必须 `Children`，`Descendants` 可能栈溢出）→ **DRIFT-017-7** 同批裁决。**选项 2（新增并行方法 `resolve_element_in`）与选项 4（把 scope 放进 `SelectorChain`）均否决**：前者制造两条入口、调用方会选错；后者把线程本地句柄塞进可学习可回写的候选数据。**D5** = 本 ADR 授权更新架构 v2 §13.1.1 两行签名 + §6.2 一句；ADR-0022 不改（E6 仍成立）。
- [2026-09-24][DECISION][src:ADR-0044] **歧义策略收敛为平台层唯一一种 fail-closed 语义**：`OnAmbiguous` 只保留 `ErrorAndAsk`，**删除** `HighestScore`。**根因** = PL-069：`HighestScore` 的语义是「取最高分候选」，但候选链的 `score` 属于**候选**（§6.2）而非命中元素 → 每个候选的命中分数恒等 → 它**永远**走 `Ambiguous`（声明了却只能报错）；且它根本不是架构 v2 §6.6 的 4 种策略之一。**归属层写清**：`require_unique` = 调用方把 `TargetAmbiguous` 直接判失败（Host / task-engine，TASK-019 / 022）；`first_by_order` = **本层不实现**（§6.6 要求「取第一个」**并且**在审计里标记，而平台层没有审计通道 → 先实现会变成静默猜，违反铁律 1）；`disambiguate_by` = Adapter 用更强的候选链（`RoleAndParent` 已实现）表达。**选项 1（保留并文档化为「等价报歧义」）与选项 3（4 种全搬进枚举）均否决**。
- [2026-09-24][DECISION][src:ADR-0045] **`#[cfg]` 分叉代码必须有「非宿主平台」编译门禁**：在 Windows 开发机上对**无 C 依赖**的 crate 跑 `cargo clippy --target x86_64-unknown-linux-gnu -p <crate> --all-targets -- -D warnings`（与 `aarch64-apple-darwin`），写进 `AGENTS.md` §6。**根因** = PL-070：`crates/platform/windows/src/unsupported.rs` 是 `#[cfg(not(windows))]`，**Windows 上从不编译**，而本地门禁全在 Windows → 结构上覆盖不到；CI run **35815168167**（`cf44b49`）= **6/8**（ubuntu + macos 红，根因 = 20 个无 `.await` 的 `async fn` + 1 处 `doc_markdown`），而**同一次提交 Windows 本地全绿**。**可行性实测**：`clippy` / `check` **不链接** → 不需要 C 交叉编译器；`--workspace` 会因 `zstd-sys` / `libsqlite3-sys` 的 `cc-rs` 失败（exit 101）→ 门禁**按 crate** 指定。**选项 1（只靠 CI）与选项 2（`#[cfg(test)]` 编译冒烟）均否决**：前者已真实翻车一次；后者会让两个同名 `WindowsPlatform` 同时编译（`E0428`）。
...
## 2026-09-26 追加（ADR-0050，Accepted）

- [2026-09-26][DECISION][src:ADR-0050] **自动化规则时间中性化 + 默认间隔 2 h + 开始时刻互斥**：① automation 可在**每天任何时间**运行，规范文字不再以「夜间」为前提 ——「当夜」= **运行日**、「晨间报告」= **阶段报告**（路径 / 目录 / 锁名 / 分支名本次**不改**，属历史命名，全量改名 = PL-089）；② ADR-0046 D3 的**默认值**由 3 h 改为 **2 h**（下限 2 h 见 ADR-0049，不变）；③ **新增硬规则**：同时存在 ≥ 2 个 automation 时，其（计划）**开始时刻必须互斥** —— 严禁两个在同一时刻开始 / 同时运行（`local` 模式共用同一主 checkout）。**锁不变**：`.nightly.lock`（章程 §11.2 / G8）仍是必需的运行时判据。**链式续建仍属未授权**（ADR-0046 D1 / 选项 4）；人类的有界计数器链提案登记为 **PL-088**（待裁决）。

## 2026-09-26 追加（ADR-0051，Accepted）

- [2026-09-26][DECISION][src:ADR-0051] **去夜间化改名（路径 + 运行期标识）**：① 文件 `docs/overnight-automation-charter.md` → `docs/automation-charter.md`；② 目录 `docs/nightly/` → `docs/automations/`；③ 锁 `.nightly.lock` → `.automation.lock`（`.gitignore` 过渡期同时忽略旧名）；④ 分支 `nightly/<date>` → `automation/<date>`。**只改现行文档 + 运行期标识，不回溯改写只追加 / 只读的历史记录**（ADR 只增不改 / LEDGER 只追加 / 任务卡正文只读）。授权例外 = README 的「工程元层」行 + 「路由」两行、MEMORY.md 的路由 / 快照两行（**只改路径**）。**「全量措辞」不在本 ADR 范围**（ADR-0050 已把规范文字时间中性化并声明「夜间 / 当夜 / 晨间」为历史命名）。详见 ADR-0051。
